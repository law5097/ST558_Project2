...
shinyUI(fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      h3("About the tabs"),
      tags$ul(
        tags$li("About Tab: gives some introductory info about the app, as well as links to resources to learn more about the data"),
        tags$li(
          "Data Selection Tab: allows you to query the API in real time with different filters that modify the API endpoint that is sent in the request, any filters you apply here will also determine the data available in the data exploration tab.",
          tags$ul(
            tags$li("Filters",
                    tags$ul(
            ...
            tags$li("Other Fields",
                    tags$ul(
                      tags$li("Record Date - The date that data was published"),
                      tags$li("Net Collections Amount - The dollar amount of transaction")
                    )
            )
          )
        ),
        tags$li("Data Exploration Tab: provides some graphical summaries of the data you selected, the plots that are available will depend on which of the data points you'd like to plot and the histogram can be faceted.")
      )
    ),
  ...
  output$plot <- renderPlot({
    ...
    # Define line plot code
      ...
      if (input$contingency_var == "record_calendar_year") {
        ggplot(data, aes(x = as.numeric(record_calendar_month), y = !!sym(input$summary_var), color = as.factor(record_calendar_year), group = record_calendar_year)) +
          geom_line(stat = "summary", fun = sum) +
          labs(title = paste(input$summary_var, "by Month and Year"), x = "Month", y = input$summary_var, color = "Year") +
          scale_x_continuous(breaks = 1:12, labels = month.name) +
          scale_y_continuous(labels = if (input$summary_var == "net_collections_amt") dollar else identity) +
          theme_minimal()
      } else {
        ggplot(data, aes(x = as.factor(record_calendar_year), y = !!sym(input$summary_var), color = !!sym(input$contingency_var), group = !!sym(input$contingency_var))) +
          geom_line(stat = "summary", fun = sum) +
